...
Also on 15 September 2026, the maintainer identified `info@ccserbie.com` as the Serbian Cultural Centre in Paris, unrelated to consular or citizen services. It is explicitly excluded from public mission data together with the duplicate Monaco row derived from it. Monaco’s existing operator-approved `ambassade.paris@mfa.rs` decision is preserved on the real embassy-derived station ID `st-nonres-mc-paris-mfa-gov-rs`. See the [Paris Cultural Centre correction](missing-recipient-recheck-2026-09-14.md#paris-cultural-centre-correction--15-september-2026).
