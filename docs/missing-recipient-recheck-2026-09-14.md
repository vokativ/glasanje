...
All 38 available mission homepages were fetched successfully. Previously inspected notices for Brasilia, Tripoli, Tunis, Ankara, Istanbul and Vukovar have unchanged body text, image markup and attachment links. Their six image files and nine Word attachments were downloaded again and match the previously inspected bytes exactly. No new recipient was found in those materials. A later maintainer review confirmed that Mostar’s shared recipient applies to Drvar; see the correction below.
...
The 84 retained candidate IDs are unchanged, with zero new IDs and zero pending groups in the selected scope. Previously approved candidates remain recorded; no AI review or promotion was needed. The six notices for Brasilia, Tripoli, Tunis, Ankara, Istanbul and Vukovar still contain no mailbox in their body. Their text, image markup and attachment links are unchanged, and fresh downloads of all six images and nine Word attachments match the previously inspected files byte-for-byte. A later maintainer review confirmed that Mostar’s shared recipient applies to Drvar; see the correction below.
...
## Third follow-up — 15 September 2026 local time

Run `20260914T225141Z-77b114c4cdef` selected the 31 mission records that remained unresolved after the eight-recipient release. The recorded Paris, Yerevan and Nairobi announcement absences and their duplicate/covered entries stayed outside the search scope. The crawler attempted 26 mission hosts, fetched 184 pages and completed with zero fetch failures or deferred stations.

No new recipient was found automatically. The durable set remains 100 candidate IDs, and the set of notice station/URL/email combinations is unchanged. The run refreshed observation times for known pages only. Tirana, Brasília, Vukovar, Tripoli, Herceg Novi, Bratislava, Tunis, Ankara and Istanbul still have current election notices with no recipient in their extracted body text.

### Drvar correction — 15 September 2026

The earlier audit interpretation was too narrow. The MFA lists the Drvar office with `https://mostar.mfa.gov.rs/` as its official website and identifies it as the consular office of the General Consulate in Mostar. The current notice on that site tells Serbian citizens residing in Bosnia and Herzegovina to submit their registration request to the General Consulate in Mostar at `gk.mostar@mfa.rs`. The maintainer confirmed that this officially published shared recipient counts for Drvar. Drvar is therefore recorded as operator-approved with the same source notice; no separate Drvar mailbox is required by the notice.

The automated report remains partial because 14 selected records reached the bounded crawl depth and eight entries have no available mission site. Those are acquisition limits, not evidence that no later notice can exist. Before the Drvar correction, public coverage remained **223 stations and 152 usable approved recipients**, so the automated recheck itself did not trigger a Firebase deployment.

The Drvar correction passed the live-deployment coverage gate against `assets/index-rZNNRuoV.js` (SHA-256 `68698ef9118e6299a1579a68b60832f3a505bb92b432d9aa23cac0fba5debe40`): **223 → 223 stations** and **152 → 153 usable approved recipients**, with Drvar as the only recipient/approval change and no removal or downgrade. Validation passed with 87 Python tests, 83 Bun tests, TypeScript checking and the production build. Firebase deployment completed successfully; at Tue, 15 Sep 2026 00:07:22 GMT, live asset `assets/index-CW5lBIGW.js` matched the tested build byte-for-byte (SHA-256 `d41e709baf36258f55f2424d12f4ea8f8d4e54cd3d7005ed9f14b643eb7e16ea`) and contained Drvar’s operator-approved `gk.mostar@mfa.rs` recipient.

### Paris Cultural Centre correction — 15 September 2026

The maintainer clarified that `info@ccserbie.com` belongs to the Serbian Cultural Centre in Paris and is unrelated to embassy consular work or services for Serbian citizens. The MFA directory places that institution inside the Paris embassy and Monaco non-resident sections, which caused the raw builder to publish it as a second Paris mission and to create a second Monaco source row. The raw scrape remains unchanged for traceability. Maintained `_excludeFromPublic` overrides now remove `st-fr-emb-main-info` and its derived `st-nonres-mc-info` record before coverage resolution, and the explicit Monaco relationship now targets the real embassy-derived `st-nonres-mc-paris-mfa-gov-rs` record.

Against the live `assets/index-CW5lBIGW.js` baseline, the intended semantic diff is **223 → 221 station records** and **153 → 153 usable approved recipients**. The unconfirmed Cultural Centre station is removed; the duplicate approved Monaco ID is replaced by approval on the existing real-embassy Monaco ID with the same `ambassade.paris@mfa.rs` recipient. No country loses a usable recipient, and `info@ccserbie.com` no longer appears anywhere in generated public data.

Validation passed with 87 Python tests, 83 Bun tests, TypeScript checking, JSON validation and the production build. Firebase deployment completed successfully. At Tue, 15 Sep 2026 02:28:37 GMT, live asset `assets/index-O-S7jFSD.js` matched the tested build byte-for-byte (SHA-256 `d5487731832d386b098e8618ffefeaa56d8b2e2c6322e0ff054dc5282336e2df`); it contains 221 station records and 153 approved recipients, contains neither excluded ID nor `info@ccserbie.com`, and retains Monaco’s operator-approved embassy recipient.

### Jakarta shared-recipient approval — 15 September 2026

The current official Jakarta notice at `https://jakarta.mfa.gov.rs/mediji/aktuelnosti` is an image headed “ОБАВЕШТЕЊЕ О ПРИЈАВИ ЗА ГЛАСАЊЕ НА ПРЕДСТОЈЕЋИМ ПАРЛАМЕНТАРНИМ ИЗБОРИМА 25. ОКТОБРА 2026. ГОДИНЕ”. It explicitly addresses citizens in Indonesia, Singapore, Thailand, Cambodia, Vietnam, Malaysia, the Philippines, Brunei Darussalam and Timor-Leste. The image prints four submission mailboxes; the maintainer selected and approved the first, `consular.jakarta@mfa.rs`, for Jakarta and all eight dependent countries. This source and choice are maintained directly because ordinary HTML extraction sees the image and attachments but not the printed mailbox text.

The release gate against live asset `assets/index-O-S7jFSD.js` (SHA-256 `d5487731832d386b098e8618ffefeaa56d8b2e2c6322e0ff054dc5282336e2df`) showed **221 → 221 stations** and **153 → 162 usable approved recipients**. Exactly Jakarta and its eight named dependent countries changed from unconfirmed to operator-approved at the same address; there were no removals, replacements or downgrades. Validation passed with 87 Python tests, 83 Bun tests, TypeScript checking and the production build. Firebase deployment completed successfully. At Tue, 15 Sep 2026 14:44:10 GMT, live asset `assets/index-BWD8Rj6B.js` matched the tested build byte-for-byte (SHA-256 `f1ec9bf4928128eaa13d0311c9b5520fd3cd6bb672ce40a99b1d4a193ec4a53f`) and exposed all nine records as operator-approved.

## Fourth follow-up and operator-approved release — 15 September 2026

After the separate Jakarta deployment and push, run `20260915T144550Z-cdd89aca81d5` checked all 33 remaining resident mission records, including Paris, Nairobi and Yerevan because a later publication supersedes a maintained absence. It attempted 26 mission hosts, fetched 396 pages, and completed with no fetch failures or deferred stations. Four selected stations reached the configured depth and seven selected records had no site; those acquisition limits do not override any published data.

Four working official pages now explicitly connect a mailbox to submission of the 2026 voter-registration request:

| Responsible mission | Approved public entries | Recipient | Current official notice |
| --- | --- | --- | --- |
| Paris | France; Monaco through its existing official relationship | `izbori.pariz@mfa.rs` | [Izbori: Prijavljivanje za glasanje u inostranstvu](https://paris.mfa.gov.rs/lat/mediji/aktivnosti/izbori-prijavljivanje-za-glasanje-u-inostranstvu) |
| Nairobi | Kenya; Tanzania; Burundi, Djibouti, Eritrea, Comoros, Rwanda, Seychelles, Somalia, South Sudan and Uganda through the existing MFA mappings | `srb.emb.kenya@mfa.rs` | [Obaveštenje o prijavi za glasanje u inostranstvu](https://nairobi.mfa.gov.rs/lat/mediji/aktivnosti/obavestenje-o-prijavi-za-glasanje-u-inostranstvu) |
| Beirut | Lebanon | `beirut.dkp@gmail.com` | [Izbori: Prijavljivanje za glasanje u inostranstvu](https://beirut.mfa.gov.rs/lat/mediji/aktivnosti/izbori-prijavljivanje-za-glasanje-u-inostranstvu) |
| Bratislava | Slovakia | `consular.bratislava@mfa.rs` | [Izbori: Prijavljivanje za glasanje u inostranstvu](https://bratislava.mfa.gov.rs/lat/mediji/najave-i-obavestenja/izbori-prijavljivanje-za-glasanje-u-inostranstvu) |

All four bare-host URLs returned HTTP 200 during direct review, and each displayed exactly the recipient above in its operative email-submission sentence. Paris and Nairobi therefore supersede their earlier `not-published` records. Monaco changes from the ordinary embassy contact to the newly published Paris election mailbox while preserving its approval and stable ID. Nairobi’s nine explicit non-resident relationships already used the same approved recipient; they gain current notice evidence without changing their address or approval. The MFA Tanzania entry points to the Nairobi mission and site, so the same notice establishes its recipient. The maintainer pre-approved these clear source-linked results and their existing resolved coverage in the instruction for this run.

The other newly or freshly observed current notices were inspected directly. Tirana, Brasília, Vukovar, Tripoli, Herceg Novi, Tunis, Ankara and Istanbul still do not provide a submission mailbox in their page body; their linked files are generic request forms where present. No recipient was inferred from an ordinary footer or contact page. Yerevan still has no mission website and retains its maintained `not-published` status with Georgia.

The pre-release semantic comparison against the previously deployed 221-station, 162-approved dataset yields **221 → 221 stations** and **162 → 167 usable approved recipients**. The five approval gains are Paris, Kenya, Lebanon, Slovakia and Tanzania. Monaco’s approved address changes to `izbori.pariz@mfa.rs`. There is no removed recipient, station deletion or approval downgrade.

Validation passed with 87 Python tests, 83 Bun tests, TypeScript checking, data generation, the production build, JSON parsing and `git diff --check`. The live baseline fetched immediately before deployment was `assets/index-BWD8Rj6B.js`, dated Tue, 15 Sep 2026 14:57:36 GMT, with SHA-256 `f1ec9bf4928128eaa13d0311c9b5520fd3cd6bb672ce40a99b1d4a193ec4a53f`; it exactly matches the documented Jakarta release snapshot. Firebase Hosting deployment completed successfully. At Tue, 15 Sep 2026 14:58:14 GMT, live asset `assets/index-DreNp9e0.js` matched the tested local build byte for byte with SHA-256 `5a5d66b596f837959d7375a1257dd5fa89f7519105e1894cee9635b74e96e636`. Direct bundle checks found all four new recipients, and generated public data reports 221 stations with 167 approved recipients.
