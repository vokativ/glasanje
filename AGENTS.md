...
- Check the visible mailbox and its `mailto:` destination separately. Official notices can contain broken links: Mostar displayed its own mailbox while linking to Skopje; Rijeka displayed `gmail.com` while linking to `gmial.com`. Record the discrepancy and use the existing operator decision for a reviewed correction; do not silently replace the visible address with the broken target. See the [remaining mission audit](docs/remaining-mission-collection-audit.md).
...
For a directly inspected announcement, store its `electionNotice` metadata in the existing station's `data/overrides.json` entry. An archived crawl packet is not required for this maintained link. Keep the actual notice URL, title, election year, observation time and extracted-email status; do not use a homepage or an old announcement as a current notice. A maintained station link takes priority over automated discovery; a station's own notice takes priority over its resolved covering mission's link. Shared publications can name separate consulates or offices (Canberra/Sydney, Rome/Malta). Read navigation labels as well as URL slugs: Vienna's election page uses a FAQ URL. See the [complete approved-recipient link audit](docs/approved-announcement-link-audit.md) for collected links, remaining gaps and the deferred internal-archive idea. A missing link does not revoke an approval.
