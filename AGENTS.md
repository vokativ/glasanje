...
Read this file first. For application structure, state ownership, script handling and offline preparation, see [architecture and maintenance contracts](docs/architecture.md). Use [election-contact operations](docs/election-contact-operations.md) for commands and schemas, [recipient decisions](docs/recipient-decisions.md) for maintained exceptions, [testing](docs/testing.md) for repeatable checks, and [deployment](docs/deployment.md) for hosting/domain maintenance. Keep the owning document current when its contract changes.

Documentation describes current behavior and repeatable procedures. Do not add dated crawl diaries, test-pass tables or release hashes to maintained docs. Keep execution evidence in ignored run/release folders and report validation in a commit or PR. Preserve structured source dates, approval evidence and the latest verified deployment baseline: those are operational data, not disposable prose. Before retiring a report, retain any still-needed decision rationale and update links, including references in maintained data notes. Git history holds superseded investigations.
...
- `info@ccserbie.com` belongs to the Serbian Cultural Centre in Paris. It is not an embassy, consular, or Serbian-citizen-services mailbox and must never be published as a mission fallback or election recipient. The MFA directory currently places the Centre inside its Paris embassy/non-resident listings; maintained exclusions remove both resulting source rows while preserving the real Paris embassy and Monaco coverage records.
...
- Check the visible mailbox and its `mailto:` destination separately. Official notices can contain broken links: Mostar displayed its own mailbox while linking to Skopje; Rijeka displayed `gmail.com` while linking to `gmial.com`. Record the discrepancy and use the existing operator decision for a reviewed correction; do not silently replace the visible address with the broken target. See [maintained source discrepancies](docs/recipient-decisions.md#excluded-contacts-and-source-discrepancies).
...
`electionContactApproval` is strictly a 3-state recipient approval enum. When an unconfirmed mission publishes an official current-election notice without an extracted submission email (`no-email-extracted`), the frontend dynamically derives an intermediate "yellow" presentation state (`notice-no-email`). It acknowledges the published announcement, provides the exact notice link, offers the mission's general contact as an unguaranteed fallback attempt, and instructs the voter to ask the mission for confirmation of receipt. This presentation state is derived at runtime; it does not alter `electionContactApproval`, does not modify canonical data schemas, and does not inflate approved recipient counts under the public deployment gate.
...
For a directly inspected announcement, store its `electionNotice` metadata in the existing station's `data/overrides.json` entry. An archived crawl packet is not required for this maintained link. Keep the actual notice URL, title, election year, observation time and extracted-email status; do not use a homepage or an old announcement as a current notice. A maintained station link takes priority over automated discovery; a station's own notice takes priority over its resolved covering mission's link. Shared publications can name separate consulates or offices (Canberra/Sydney, Rome/Malta). Read navigation labels as well as URL slugs: Vienna's election page uses a FAQ URL. Read current links from maintained data and exceptions from [recipient decisions](docs/recipient-decisions.md). Optional internal archiving of manually inspected notices is a future consideration, not a prerequisite. A missing link does not revoke an approval.

An explicit maintainer statement that a mission has no current announcement is recorded as `electionNoticeStatus: "not-published"` in its existing override, with the election/year and decision context in `_electionNoticeNote`. Show this status for the mission and its already-resolved covered countries, without changing recipient approval. Do not infer it from a failed crawl. A later actual announcement takes precedence over this recorded absence.
...
A country may have a resident embassy while its consular work is assigned elsewhere. Latvia's MFA entry explicitly assigns consular matters to Sweden. For this tool, the maintained `isResident: false` correction and official coverage relationship resolve its existing entries to Stockholm without deleting or renaming their stable IDs. This describes the voter-registration destination, not the absence of diplomatic relations or a resident embassy.
